...
async def main():
    ...
    lose_text2 = button_font.render("Press Backspace to return to Menu", True, [255, 255 ,255], [0, 0, 0])
    ...
    back_text = button_font.render("Back", True, [255, 255 ,255], [0, 0, 0])
    back_text.set_colorkey([0, 0, 0])
    
    button_font.align = pygame.FONT_CENTER
    code_credit = button_font.render("Code and Game Design by Two119", True, [255, 255 ,255], [0, 0, 0])
    art_credit = button_font.render("Art by MasterGemu", True, [255, 255 ,255], [0, 0, 0])
    music_credit = button_font.render("Music and SFX by travsul", True, [255, 255 ,255], [0, 0, 0])
    button_font.align = pygame.FONT_LEFT
    credit_screen = cover.copy()
    code_credit.set_colorkey([0, 0, 0])
    art_credit.set_colorkey([0, 0, 0])
    music_credit.set_colorkey([0, 0, 0])
    credit_screen.blit(art_credit, [utils.center_pos(art_credit)[0], utils.center_pos(art_credit)[1]+32])
    credit_screen.blit(code_credit, [utils.center_pos(code_credit)[0], utils.center_pos(code_credit)[1]-32])
    credit_screen.blit(music_credit, [utils.center_pos(music_credit)[0], utils.center_pos(music_credit)[1]+96])
    ...
    class WaveManager:
        def __init__(self):
            ...
            self.waves = []
            for i in range(6):
                self.waves.append([[i*228, 360-self.wave_sprites[0].get_height()+50], 0])
            self.delay = 0
        def update(self):
            self.delay+=1
    ...
    def credit(any):
        ...
        game_state = 2
    def end(any):
        if not __import__("sys").platform == "emscripten":
            raise SystemExit
    def back(any):
        global game_state
        game_state = 0
    ...
    back_button = Button([utils.center_pos(button_sprites.sheet[0][0])[0],utils.center_pos(button_sprites.sheet[0][0])[1]+200], [button_sprites.sheet[0][i].copy() for i in range(len(button_sprites.sheet[0]))], [back, 1])
    ...
    back_button.textures[0].blit(back_text, [(back_button.textures[0].get_width()-back_text.get_width())/2, (back_button.textures[0].get_height()-back_text.get_height())/2])
    ...
    while not cr.event_holder.should_quit:
        ...
        if game_state == 1:
                ...
                if player.fishing and player.recreated:
                    ...
                    if player.rope is not None:
                        if player.rope.lowest_point is not None:
                            ...
                            pygame.draw.circle(cr.screen, [155, 173, 183], player.rope.lowest_point, 5)
            ...
            if screenshot != None:
                ...
                if pg.K_BACKSPACE in held_keys:
                    game_state = 0
                    player.__init__()
                    fish_manager.__init__()
                    screenshot = None
            ...
            cr.screen.blit(credit_screen, [0 ,0])
            back_button.update()
